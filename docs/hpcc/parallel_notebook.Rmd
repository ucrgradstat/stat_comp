...
The easiest way to parallelize is to use the `mclapply()` function. All you need to do is change the `lapply()` function to `mclapply()` and add the argument `mc.cores =  ncores`. The argument `mc.cores` specifies the number of cores for parallelization.
...
The `clusterExport()` function is needed to export user-created, package functions, or data to the cluster.
You will need to specify the cluster to export the information (`cl`) and the name of the functions or data in a character vector. 


#### `parLapply()`

The `parLapply()` is the parallel computing component. It is used the same way as the `lapply()`. The only difference is that you will need to specify the cluster (`cl`) as the first argument. The second argument goes as the `lapply()` function.
