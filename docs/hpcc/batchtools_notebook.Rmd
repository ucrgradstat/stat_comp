...
R is designed to only use one cpu (or core) when running tasks^[Testing]. However, you may have access to a computer cluster[^1] that allows you to access more RAM and cpus. The use of more than one cpu is known as parallel computing in R. The goal of this tutorial is to provide the basics of using the `batchtools` package and utilizing more cores in a cluster.
...
This tutorial has an R script that you can use in the cluster. You can access R script [here](batchtools.R)
...
The function below process the data and implements the `lm()` to the data. This is the function that will be used in `batchtools`. First, the function obtains the number of data sets it will process. Second, it creates the `lm_coef()` function which applies the `lm()` function to the data^[User created functions needs to be created within the function to be loaded in `batchtools`]. Third, a parallel processor is applied to go through the data ^[For more information about parallel processing with the `parallel` package, visit [here](parallel_notebook.nb.html)]. Lastly, the results are converted to a matrix and returned as the output.   
